...
Ext.onReady(function (){
	...
	function add() {
	    ...
	    var win = new Ext.Window({
	        ...
	        items: [{
	            ...
	            items: [
	               { fieldLabel: "角色名", name: "roleName", blankText: "请输入角色名" },
	               { fieldLabel: "角色类型", name: "roleType", blankText: "请输入角色类型" },
	               { fieldLabel: "角色等级", name: "roleLevel", blankText: "请输入角色等级",xtype: "numberfield",minValue:level,negativeText:'角色等级不可高于你'}
	            ]
	        }],
	        buttons: [
	             {
	...
	function edit() {
	    ...
	    var id,roleName,roleType,roleLevel;
	    if (selRecords.length >= 1) {
	    	console.log(selRecords[0].get("roleName"));
	        id = selRecords[0].get("id");
	        roleName = selRecords[0].get("roleName");
	        roleType = selRecords[0].get("roleType");
	        roleLevel = selRecords[0].get("roleLevel");
	    ...
	    var win = new Ext.Window({
	        ...
	        items: [{
	            ...
	            items: [
	               ...
	               { fieldLabel: "角色名", name: "roleName", blankText: "请输入角色名",value:roleName },
	               { fieldLabel: "角色类型", name: "roleType", blankText: "请输入角色类型",value:roleType },
	               { fieldLabel: "角色等级", name: "roleLevel", blankText: "请输入角色等级",xtype: "numberfield",minValue:level,negativeText:'角色等级不可高于你',value:roleLevel}
	            ]
	                    ...
	                    Ext.getCmp("form1").getForm().submit(
	        					 {
	        						 ...
	        						 waitMsg:'请稍候',
	        						 waitTitle:'正在提交',
	        						 url:'${ctx}/system/role/updateRole',
	        						 success:function(form,action){
	        							 Ext.MessageBox.show({					
	                                         width:150,
	                                         title:"更新成功",
	    ...
	    var ids = "";
	            ...
	            Ext.Msg.confirm("提示", "确定要删除吗?", function (btn) {
	                if (btn == "yes") {
	                    ...
	                    Ext.Ajax.request({
	                        url: "${ctx}/system/role/betchDelRole/"+ids,
	                        /* params: {
	                            "ids": ids
	                        }, */
	...
	var dataGrid=Ext.create('Ext.grid.Panel', {
	    ...
	    columns: [
	              rn,
	        { flex:3,text: '角色名',  dataIndex: 'roleName' },
	        {  flex:3,text: '角色类型', dataIndex: 'roleType'},
	        {  flex:3,text: '角色等级', dataIndex: 'roleLevel'}
	    ],
	    viewConfig: {
